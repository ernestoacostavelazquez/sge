...
@Component({
  selector: 'app-lineas',
  standalone: true,
  imports: [ReactiveFormsModule,HttpClientModule,JsonPipe,NgFor,NgIf],
  ...
  getLineas() {
    this.http.get('http://localhost:3000/api/Lineas').subscribe((res: any) => {
      if (Array.isArray(res.data)) {
        ...
        // Inicializamos el DataTable después de cargar los datos
        setTimeout(() => {
          this.initializeDataTable();
        }, 0); // Esperamos que Angular renderice los datos antes de inicializar
      } else {
